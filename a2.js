...
bot.on("spawn", async () => {
    ...
    for (const z in schematic) {
        ...
        for (const x in schematic[z]) {

            var block = schematic[z][x]

            if (z % 2 == 0) { // Every second row


                if (!bot.pathfinder.isMoving()) {

                    bot.pathfinder.setGoal(new GoalNear(128, bot.player.entity.position.y, block.z, 0))
                }
                ...
                if (!bot.pathfinder.isMoving()) {
                    bot.pathfinder.setGoal(0, bot.player.entity.position.y, block.z, 0)
                }
